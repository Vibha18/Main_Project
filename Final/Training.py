...
for c in positive_comments:
    all_comments.append([c, "Positive"])

for c in negative_comments:
    all_comments.append([c, "Negative"])

for c in neutral_comments:
    all_comments.append([c, "Neutral"])
...
shuffled_df.to_csv('before_preprocessing.csv', index=False)
v=shuffled_df.iloc[:,0:]
data_file=v
...
def tokenize_text(text):
    if isinstance(text, str):
        ...
        return [] 
data_file['text'] = data_file['text'].apply(tokenize_text)
...
df=data_file
data_file.to_csv('training_comments.csv', index=False)